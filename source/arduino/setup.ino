...
// not enough memory to do 4 different flag icons on preset screen
lv_img_dsc_t my_flag_img[1];
uint8_t my_flag_img_data[1][96*72*3]; // 20736 bytes


void SetFlag(char *countrycode, uint16_t PresetStation)
{ char lowercasecode[8];
  ...
  Serial.printf("Setflag with %s Preset %d\n", path, PresetStation);

  my_flag_img[PresetStation].header.always_zero = 0;
  my_flag_img[PresetStation].header.w = 96;
  my_flag_img[PresetStation].header.h = 72;
  my_flag_img[PresetStation].header.cf = LV_IMG_CF_TRUE_COLOR_ALPHA;
  my_flag_img[PresetStation].data = my_flag_img_data[PresetStation];

  SD_MMC.begin("/sdcard", true, false);
  File fp = SD_MMC.open(path, FILE_READ);
  if(fp!=NULL)
  { fp.read((uint8_t *)my_flag_img_data[PresetStation], 4); // header, basically skipping it, header is already filled above
    fp.read((uint8_t *)my_flag_img_data[PresetStation], sizeof(my_flag_img_data[PresetStation])); // header
    fp.close();
  ...
  if(PresetStation==0)
  { lv_img_set_src(ui_Home_Flag, &my_flag_img[PresetStation]);
    lv_img_set_src(ui_Database_Flag, &my_flag_img[PresetStation]);
    lv_img_set_src(ui_HomeFlagToStore, &my_flag_img[PresetStation]);
    lv_img_set_src(ui_ClockFlag, &my_flag_img[PresetStation]);
  }
  else
  { //if(PresetStation==1)lv_img_set_src(ui_PresetFlag1, &my_flag_img[PresetStation]);
    //if(PresetStation==2)lv_img_set_src(ui_PresetFlag2, &my_flag_img[PresetStation]);
    //if(PresetStation==3)lv_img_set_src(ui_PresetFlag3, &my_flag_img[PresetStation]);
    //if(PresetStation==4)lv_img_set_src(ui_PresetFlag4, &my_flag_img[PresetStation]);
  }  
...
{ static int actbatteryvoltage = -1;
  ...
  { if(actbatteryvoltage != -newbatteryvoltage)
    { actbatteryvoltage = -newbatteryvoltage;
      ...
      if(actbatteryvoltage>=410)actbatterypercentage = 100;
      else if(actbatteryvoltage>=406)actbatterypercentage = 90; 
      else if(actbatteryvoltage>=398)actbatterypercentage = 80; 
      else if(actbatteryvoltage>=392)actbatterypercentage = 70; 
      else if(actbatteryvoltage>=387)actbatterypercentage = 60; 
      else if(actbatteryvoltage>=375)actbatterypercentage = 50; 
      else if(actbatteryvoltage>=367)actbatterypercentage = 40; 
      else if(actbatteryvoltage>=360)actbatterypercentage = 30; 
      else if(actbatteryvoltage>=345)actbatterypercentage = 20; 
      else if(actbatteryvoltage>=330)actbatterypercentage = 10;
      else if(actbatteryvoltage>=200)actbatterypercentage = 1;
      else actbatterypercentage = 0;
        
      sprintf(content, "%d%%", actbatterypercentage);
      lv_label_set_text(ui_BatteryVoltage, content);
    }
    return;
  }
  else
  { if(newbatteryvoltage>1)actbatteryvoltage = newbatteryvoltage; // if a true voltage level is send
  }
